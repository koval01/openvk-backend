...
#[tokio::test]
async fn echoes_a_valid_incoming_trace_id() {
    let (base, _) = start_app().await;
    ...
    let response = reqwest::Client::new()
        ...
        .await
        .unwrap();
    assert_eq!(header(&response, TRACE_HEADER.as_str()), incoming);
}
...
async fn error_body_carries_the_same_trace_id_as_the_header() {
    let (base, _) = start_app().await;
    let incoming = TraceId::new().to_string();
    let response = reqwest::Client::new()
    ...
    assert_eq!(error.trace_id, incoming);
}
